...
Fine-tunes Romanian [ReaderBench RoBERT](https://huggingface.co/readerbench) on MOROCO for six news categories (culture, finance, politics, science, sports, tech). Outputs and saved configs: `outputs/` (`outputs/<run>/config.toml` per run).
...
Backbones: `readerbench/RoBERT-small`, `readerbench/RoBERT-base`. Full runs: 3 epochs, `2e-5`. LoRA runs: 5 epochs, `r=8`, `lora_alpha=16`, `lora_dropout=0.1`, `1e-4`. Batch size 16 for all (see `config.toml`).

Parameter counts match `outputs/<run>/eval_results.json`.

| Run directory | Backbone | Adaptation | Trainable params | Total params | Trainable % |
|---------------|----------|------------|------------------|--------------|-------------|
...
**Hardware:** MacBook Pro 14", Apple M4 Pro (12 CPU, 16 GPU), 24 GB RAM. MPS when available; batch size 16 from `config.toml`.

---

MOROCO path: `config.toml` → `[dataset.moroco]`. `outputs/` does not record library versions, random seeds, or which checkpoint was evaluated.
